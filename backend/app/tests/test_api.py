...
async def test_upload_document(client):
    """POST /api/documents/upload deve fazer upload, chamar o serviço de ingestão e salvar no banco de dados."""
...
async def test_generate_content_success(client, test_db_session):
    ...
    mock_generate = AsyncMock(
        return_value=(
            '# Resumo\nFórmula: $\\frac{1}{2}$',
            'Fórmula: fração com numerador 1 e denominador 2',
        )
    )
    mock_tts = AsyncMock(return_value='test-audio.mp3')

    with (
        patch('app.main.llm_service.generate_content', mock_generate),
...
async def test_get_audio_file_success(client):
    ...
    with (
        ...
        mock_fileresponse.return_value = Response(
            content=b'fake-audio-content', media_type='audio/mpeg'
        )
